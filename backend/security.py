...
# A2 (R2-R4 race-fix): asyncio.Lock pra serializar acesso aos dicts
# compartilhados entre coroutines. Em single-thread asyncio, codigo sync
# sem await ja e atomic, mas as ops aqui podem rodar entre awaits do
# middleware/handler que as chama. Lock e barato e garante consistencia
# mesmo se algum dia uma das funcoes ganhar await internamente.
#
# Limitacao: vale apenas pra single-process. Multi-worker exige Redis
# (D7 no roadmap).
...
async def record_event(ip: str, event_type: str) -> None:
...
def _block_ip_unlocked(ip: str, reason: str) -> None:
    """Bloqueia IP. Caller DEVE estar segurando _state_lock."""
    _blocked_ips[ip] = time.time() + BLOCK_DURATION
    logger.warning("SECURITY: IP %s bloqueado por %ds — %s", ip, BLOCK_DURATION, reason)


def _should_alert_unlocked(key: str) -> bool:
    """Anti-spam: retorna True se pode alertar (cooldown expirou).
    Caller DEVE estar segurando _state_lock."""
    now = time.time()
...
async def analyze_request(ip: str, path: str, status_code: int, method: str) -> dict | None:
    ...
    A2 (R2-R4): toda manipulacao de _events / _blocked_ips / _alerted acontece
    com _state_lock segurado pra atomicizar check + record + maybe-alert.
    """
    if not SECURITY_ENABLED:
        return None

    # Classifica o evento (record_event ja pega lock internamente)
    if status_code == 404:
        await record_event(ip, "404")
        ...
        await record_event(ip, "auth_fail")
    elif status_code == 429:
        await record_event(ip, "rate_limited")

    # Detecao + block + alert: tudo dentro de um critical section atomico.
    async with _state_lock:
        now = time.time()
        ...
        if n404 >= SCAN_THRESHOLD:
            _block_ip_unlocked(ip, f"scan detectado ({n404} 404s em 1 min)")
            if _should_alert_unlocked(f"scan:{ip}"):
                return {"type": "scan", "ip": ip, "count": n404, "window": "1 min"}

        # Deteccao 2: Brute force (muitos auth failures em 5 min)
        n_auth_5m = sum(1 for _, e in recent_5m if e == "auth_fail")
        if n_auth_5m >= BRUTE_THRESHOLD:
            _block_ip_unlocked(ip, f"brute force detectado ({n_auth_5m} auth failures em 5 min)")
            if _should_alert_unlocked(f"brute:{ip}"):
                return {"type": "brute_force", "ip": ip, "count": n_auth_5m, "window": "5 min"}

        # Deteccao 3: Credential stuffing (muitos 401/403 em 1 min)
        n_auth_1m = sum(1 for _, e in recent_1m if e == "auth_fail")
        if n_auth_1m >= AUTH_FAIL_THRESHOLD:
            _block_ip_unlocked(ip, f"credential stuffing ({n_auth_1m} em 1 min)")
            if _should_alert_unlocked(f"stuffing:{ip}"):
                return {"type": "credential_stuffing", "ip": ip, "count": n_auth_1m, "window": "1 min"}
...
async def handle_honeypot(ip: str, path: str) -> dict:
    ...
    async with _state_lock:
        # record_event inline (evita re-pegar o lock)
        now = time.time()
        _events[ip].append((now, "honeypot"))
        _events[ip] = [(t, e) for t, e in _events[ip] if now - t < 600]
        _block_ip_unlocked(ip, f"honeypot hit: {path}")
        logger.warning("HONEYPOT: %s acessou %s — IP bloqueado", ip, path)
        if _should_alert_unlocked(f"honeypot:{ip}"):
